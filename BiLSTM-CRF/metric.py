...
# Prefer seqeval for robust NER metrics when available
try:
...
def f1_score(y_true, y_pred, mode='dev'):
    ...
    Uses seqeval if available (recommended). Falls back to the project
    internal span-based computation if seqeval is not installed.
    """
    if _SEQEVAL:
        # seqeval expects lists of lists of tags
        # For dev mode we return a single global F1
        if mode == 'dev':
            return seq_f1(y_true, y_pred)
        else:
            # For test mode return per-label f1 dict and overall f1
            report = seq_classification_report(y_true, y_pred, output_dict=True)
            # report contains entries like 'ORG': {'precision':..., 'recall':..., 'f1-score':..., 'support':...}
            f_score = {}
            for label in config.labels:
                # seqeval uses label names directly only for entity-level entries
                if label in report:
                    f_score[label] = report[label]['f1-score']
                else:
                    f_score[label] = 0.0
            overall_f1 = seq_f1(y_true, y_pred)
            return f_score, overall_f1
    else:
